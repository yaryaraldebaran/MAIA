...
public class MainSyncTaskForAsyncAsynChild {
    ...
    public void asyncParent(final int intAsyncParent)
    {
        ...
        new Thread(new Runnable() {
            public void run()
            {
            	for(int i=0;i<4;i++)
            	{
            		asyncChild(intAsyncParent,i);//yang ini jalan terus, total 10+5 detik
            		synchronousTaskForAsync(intAsyncParent,i); //delay 5 detik
            		try {
    ...
    public void synchronousTaskForAsync(int intAsyncParent,int intLoopAsyncParent)
    {
    	for(int i=0;i<5;i++)
    	{
    		intLoop++;
    		try {
				Thread.sleep(1000);//jadi 1 detik
			} catch (InterruptedException e) {
				System.out.println("Interrupted Exception in Synchronous Task "+e);
			}
    ...
    {	
    	MainSyncTaskForAsyncAsynChild obj = new MainSyncTaskForAsyncAsynChild();
